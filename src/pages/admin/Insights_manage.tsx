...
const InsightsManage = () => {
  ...
  const [existingImages, setExistingImages] = useState<string[]>([]); // stored paths kept
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.get(`${BASE_URI}/api/insights/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // POST for both create and update (PHP can't read multipart on PUT)
      const url = id ? `${BASE_URI}/api/insights/${id}` : `${BASE_URI}/api/insights`;
      ...
      const msg = id ? "Insight updated successfully!" : "Insight added successfully!";
      navigate("/admin/insights", { state: { message: msg } });
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white shadow rounded-lg p-6">
      <Toaster />
      <h1 className="text-2xl font-bold mb-6">{id ? "Edit Insight" : "Add New Insight"}</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="text" className="w-full border rounded px-3 py-2" value={formData.author} onChange={(e) => setFormData({ ...formData, author: e.target.value })} required />
        </div>
        <div>
          <label className="block font-semibold mb-1">Tags (comma separated)</label>
          <input type="text" placeholder="Air Freight, India, Export" className="w-full border rounded px-3 py-2" value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })} />
        </div>
        <div>
          <label className="block font-semibold mb-1">Description (excerpt)</label>
        ...
        >
          {saving ? "Saving..." : id ? "Update Insight" : "Add Insight"}
        </button>
      </form>
    </div>
  );
